...
public class BikeService {
    ...
    public Flux<BikeDto> fetchAndSaveBikes() {
        int totalCount = 3500;
        int batchSize = 500; // ⚠️ 1회에 500개씩 처리
        int numBatches = (int) Math.ceil((double) totalCount / batchSize);

        return Flux.range(0, numBatches)
                .concatMap(batchIndex -> {
                    int start = batchIndex * batchSize + 1;
                    int end = Math.min((batchIndex + 1) * batchSize, totalCount);
                    System.out.println("API 호출: 범위 " + start + "~" + end);
                    return apiClient.fetchBikeDataJson(start, end)
                            .doOnNext(json -> System.out.println("API 응답 수신: 범위 " + start + "~" + end));
                })
                .flatMap(batchJson -> {
                    try {
...
/* 지후님 코드 */
//    public Flux<BikeDto> fetchAndSaveBikes() {
//        int totalCount = 3500; // totalCount 고정
//        return Flux.just(
